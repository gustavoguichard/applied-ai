...
const labelNames = ['premium', 'medium', 'basic'] as const
const tensorLabels = [
  [1, 0, 0], // premium - Erick
  [0, 1, 0], // medium - Ana
  [0, 0, 1] // basic - Carlos
...
const normalizedPersonTensor: number[][] = [
  [
    0.2, // Normalized age
    1, // Blue color
    0, // Red color
...
async function predict(model: tf.Sequential, person: number[][]) {
  const tfInput = tf.tensor2d(person)
  const prediction = model.predict(tfInput) as tf.Tensor
